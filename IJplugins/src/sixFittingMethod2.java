...
public class sixFittingMethod2 {

	ImagePlus imp;

	public static int[][] imageMatrix;

	public double[] theFitter(ImageProcessor ip, int iROI, double abest,
			double zbest, double xbest, double ybest, double wxbest,
			double wybest) {
		...
		// int[][] imgMatrix =imgToMatrix(imp);
		int imgWidth = imageMatrix.length;

		// div = 4 corresponds to adding bestvalue + (-50%, -25%, 0, 25%,
		// +50%)*bestvalue for i=(-2,-1,0,1,2)
		double div = 8.00; // the only difference from SixFitting1, except
							// passing arguments
		double da = 1 / div;
		double dz = 1 / div;
		double dx = 4 / div; // for div=8: -1 to 1 pixels from xbest=0
		double dy = 4 / div;
		double dwx = 4 / div; // wxbest=2: {1 to 5}, wxbest=1: {0.5 to 2.5},
								// wxbest=0.5: {0.25 to 1}
		double dwy = 4 / div;
	...
	public double fitGaussian(double noise, double amplitude, double centreX,
		...
		return chi;// return the sume of differences squared.
	}
